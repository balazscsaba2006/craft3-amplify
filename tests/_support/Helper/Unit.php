...
class Unit extends Module
{
    ...
    {
        $mockModule = $this->getMock($test, Amplify::class);
        $mockModelMapper = $this->getMock($test, Component::class);
        $mockModule
            ->expects($test->any())
            ->method('__get')
            ->willReturnMap([
    ...
    /**
     * Get a preconfigured mock twig extension.
     *
     * @param TestCase $test
     *
     * @return Mock|Amplify
     */
    private function getMockTwigExtension(TestCase $test)
    {
        $mockModule = $this->getMock($test, Amplify::class);
        $mockTwigExtension = $this->getMock($test, TwigExtensions::class);
        $mockModule
            ->expects($test->any())
            ->method('__get')
            ->willReturnMap([
     ...
     * @return Mock
     */
    private function getMock(TestCase $test, string $class): Mock
    {
        return $test
